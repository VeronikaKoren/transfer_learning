...
%% inputs: "s1" and "s2" are spike counts 2 conditions, of the form (number of trials x number of neurons)
...
for cv=1:ncv
    ...
    for c=1:length(Cvec)                % range of C-parameters
        ...
        for m=1:nfold
            ...
            try
                ...
                % train the SVM model
                svm_model=fitcsvm(xc_train,labc_train,'KernelFunction','linear','BoxConstraint',Cvec(c),'Standardize',true); % train linear SVM
                % validate the SVM
                class=predict(svm_model,xc_val);
    ...
    try
        ...
        % train the SVM model
        svm_model=fitcsvm(train,label,'KernelFunction','linear','BoxConstraint',Cvec(c),'Standardize',true); % train linear SVM
        % validate the SVM
        class=predict(svm_model,validate); 
