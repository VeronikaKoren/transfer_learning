...
% compute ccg noise within the group of neurons with strong (weak) weights
% cross-correlation function (ccg = ccg_raw - ccg_trial_invariant)
% concatenated trials from conditions CNM and INM
...
nperm=1000;                                                                       % number of permutatios
...
%% permutation test for difference in synchrony 

d=mean(g1mat(:,K))-mean(g2mat(:,K));

% concatenate pairs across sessions
p1=cell(nperm,1);
p2=cell(nperm,1);
for perm=1:nperm
...
d0=cellfun(@(x,y) mean(x(:,taumax))-mean(y(:,taumax)),p1,p2); % compute the synchrony, averaged across pairs
pval=sum(d<d0)/nperm;
...
for tau=1:ntau
    ...
    dc0=cellfun(@(x,y) mean(sum(x(:,taumax-tau:taumax+tau),2))-mean(sum(y(:,taumax-tau:taumax+tau),2)),p1,p2);
...
if saveres==1
    ...
    filename=['ccg_info_',sprintf('%1.0i',K)];
