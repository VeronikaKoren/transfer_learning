...
%%
psth_tar=psth_sess(:,1);
psth_test=psth_sess(:,2);

figure('visible','off')
hold on
plot(nanmean(psth_test{1}))
plot(nanmean(psth_test{2}))

%% save

if saveres==1
    savefile='/home/veronika/synced/transfer_result/signal/psth/';
    savename=['psth_',namea{ba}];
    save([savefile,savename],'psth_tar','psth_test')
end
