...
for p=1:np
    ...
    loadname=['jitter_window_',sprintf('%1.0i',d_range(p))];
    load(loadname)
...
for p=1:np
    ...
    title(['window = ', sprintf('%1.0i', d_range(p)), 'ms'],'Fontweight','normal','FontName','Arial','fontsize',fs)
...
h2 = ylabel ('population signal (a.u.)','units','normalized','Position',[-0.1,0.5,0],'FontName','Arial','fontsize',fs);
set(gca,'Visible','off')
set(h2,'visible','on')
set(h1,'visible','on')
